...
@Component({
  selector: 'app-information-setting',
  templateUrl: './information-setting.component.html',
  styleUrls: ['./information-setting.component.scss'],
})
export class InformationSettingComponent implements OnInit {
  ...
  initFormValue() {
    this.formUserInfor = new FormGroup({
      userName: new FormControl({ value: this.userInfor.userName, disabled: true }, [
        Validators.required,
  ...
  selectProvince(dataSelect: any) {
    ...
    let province = this.provinces.find((item: any) => item.name == dataSelect.value)
    if (province) {
  ...
  selectDistrict(dataSelect: any) {
    let district = this.districts.find((item: any) => item.name == dataSelect.value)
    if (district) {
